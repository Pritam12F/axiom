...
import { TrafficLights } from "@/components/fleet/traffic-lights"
  ...
  return (
    <aside className="flex h-full w-56 shrink-0 flex-col border-r border-sidebar-border bg-sidebar/95 backdrop-blur-xl supports-[backdrop-filter]:bg-sidebar/80">
      ...
      <div className="mt-5 px-4">
        <p className="px-0.5 text-[11px] font-medium uppercase tracking-wide text-sidebar-foreground/45">Groups</p>
      </div>
        ...
        <DropdownMenuContent align="start" side="top" className="w-48">
          <DropdownMenuGroup>
            ...
            <DropdownMenuItem onClick={() => setTheme("light")}>
              <Sun />
              Light
              {theme === "light" && <Badge variant="secondary" className="ml-auto text-[10px]">On</Badge>}
            </DropdownMenuItem>
            <DropdownMenuItem onClick={() => setTheme("dark")}>
              <Moon />
              Dark
              {theme === "dark" && <Badge variant="secondary" className="ml-auto text-[10px]">On</Badge>}
            </DropdownMenuItem>
            <DropdownMenuItem onClick={() => setTheme("system")}>
              <Monitor />
              System
              {theme === "system" && <Badge variant="secondary" className="ml-auto text-[10px]">On</Badge>}
            </DropdownMenuItem>
          </DropdownMenuGroup>
          <DropdownMenuSeparator />
          <DropdownMenuItem>
