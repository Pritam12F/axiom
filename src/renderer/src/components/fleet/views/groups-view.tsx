...
import { ProjectTray } from "@/components/fleet/groups/project-tray"
import { GroupTabs } from "@/components/fleet/groups/group-tabs"
import { MemberChip } from "@/components/fleet/groups/member-chip"
import { KanbanColumn } from "@/components/fleet/groups/kanban-column"
import { TaskCard } from "@/components/fleet/groups/task-card"
...
export function GroupsView({ groupId }: { groupId?: string }) {
  ...
  const [dragOverlay, setDragOverlay] = React.useState<{ type: "project" | "task"; id: string } | null>(null)
    ...
    return (
      <Empty className="py-16">
        <EmptyHeader>
          ...
          <EmptyDescription>Create a group to start assigning projects and agents.</EmptyDescription>
        </EmptyHeader>
      </Empty>
    )
  }
  ...
  function handleDragStart(e: DragStartEvent) {
    ...
    if (id.startsWith("project-")) setDragOverlay({ type: "project", id: id.replace("project-", "") })
  ...
  const draggedProject = dragOverlay?.type === "project" ? projects.find((p) => p.id === dragOverlay.id) : null
  const draggedTask = dragOverlay?.type === "task" ? tasks.find((t) => t.id === dragOverlay.id) : null
  ...
  return (
    <DndContext onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
      <div className="flex flex-col gap-4 p-5">
        ...
        <GroupTabs groups={groups} activeId={activeGroup.id} onSelect={(id) => setView({ name: "groups", groupId: id })} />
        ...
        {assignedProject ? (
          ...
          <Empty className="py-12">
            <EmptyHeader>
              ...
              <EmptyDescription>Drag a project from the tray above onto this group.</EmptyDescription>
            </EmptyHeader>
          </Empty>
        )}
      </div>
      ...
      <DragOverlay>
        ...
        {draggedTask && <TaskCard task={draggedTask} agent={agents.find((a) => a.id === draggedTask.assigneeAgentId)} />}
      </DragOverlay>
    </DndContext>
  )
}
