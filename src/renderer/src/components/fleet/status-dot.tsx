...
export const STATUS_META: Record<AgentStatus, { label: string; className: string; pulse?: boolean }> = {
...
export function StatusDot({ status, className }: { status: AgentStatus; className?: string }) {
  ...
  return (
    <span className="relative inline-flex size-2" aria-hidden="true">
      <span className={cn("absolute inset-0 rounded-full", meta.className, meta.pulse && "status-pulse", className)} />
    </span>
  )
}

export function StatusLabel({ status, showDot = true }: { status: AgentStatus; showDot?: boolean }) {
  const meta = STATUS_META[status]
  return (
