      ...
      <div className="flex items-center justify-between">
        ...
        <Button
          ...
          onClick={() => {
            setTookOver((v) => !v)
            toast(
              tookOver ? 'Released control back to agent' : 'You have taken over this terminal',
              {
                description: tookOver
                  ? undefined
                  : `${agentName} is paused while you drive the session.`
              }
            )
          }}
        >
          ...
          {lines.map((line, i) => (
            <div
              ...
              className={
                line.startsWith('$')
                  ? 'text-[#7dd3fc]'
                  : line.includes('error') || line.includes('failing') || line.includes('✗')
                    ? 'text-[#fda4af]'
                    : line.includes('✓') || line.includes('passing')
                      ? 'text-[#86efac]'
                      : ''
              }
            >
