...
import { ChevronRight, FileEdit, FlaskConical, GitCommit, MessageSquare, Sparkles, Terminal as TerminalIcon } from "lucide-react"
...
const KIND_META: Record<EventKind, { icon: typeof Sparkles; label: string; className: string }> = {
  ...
  "file-edit": { icon: FileEdit, label: "File edit", className: "text-status-sleeping bg-status-sleeping/10" },
  git: { icon: GitCommit, label: "Git", className: "text-status-running bg-status-running/10" },
  test: { icon: FlaskConical, label: "Test", className: "text-status-approval bg-status-approval/10" },
...
export function ActivityTab({ events }: { events: AgentEvent[] }) {
  if (events.length === 0) {
    return <p className="p-6 text-center text-sm text-muted-foreground">No activity recorded yet.</p>
  }
      ...
      {events.map((event, i) => {
        ...
        return (
          <li key={event.id} className="relative flex gap-3 pb-4 last:pb-0">
            {i < events.length - 1 && <span className="absolute left-[15px] top-7 h-[calc(100%-20px)] w-px bg-border" />}
            <div className={cn("z-10 flex size-[30px] shrink-0 items-center justify-center rounded-full", meta.className)}>
            ...
            <Collapsible className="min-w-0 flex-1">
              <CollapsibleTrigger className="group flex w-full items-start justify-between gap-2 text-left">
                <div className="min-w-0">
                  <p className="text-[13px] font-medium leading-snug text-foreground">{event.title}</p>
                  <TimeAgo iso={event.timestamp} className="tabular block text-[11px] text-muted-foreground" />
                </div>
                {event.detail && (
